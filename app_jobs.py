...
def search_remotive(keywords: str, category: str = "") -> list[dict]:
    """Remotive — API gratuite, offres 100% remote."""
    try:
        params = {"search": keywords, "limit": 20}
        if category:
            params["category"] = category
        r = requests.get("https://remotive.com/api/remote-jobs", params=params, timeout=10)
        if r.status_code != 200:
            return []
        jobs = []
...
def search_arbeitnow(keywords: str) -> list[dict]:
    """Arbeitnow — API 100% gratuite, sans clé, emplois Europe + Remote."""
    try:
        r = requests.get(
        ...
        for item in r.json().get("data", [])[:15]:
            jobs.append({
                ...
                "url": item.get("url", ""),
                "salary": "",
                "contract": "Remote" if item.get("remote") else "",
                "posted": item.get("published_at", "")[:10],
            })
        return jobs
    except Exception as e:
        st.warning(f"Arbeitnow : {e}")
        return []
...
def aggregate_jobs(keywords, location, country,
                   adzuna_id, adzuna_key, rapidapi_key) -> list[dict]:
    """Lance toutes les recherches et agrège les résultats."""
    all_jobs = []
    with st.spinner("🔍 Recherche en cours sur toutes les plateformes..."):
        prog = st.progress(0, text="Arbeitnow (gratuit, sans clé)...")
        arb = search_arbeitnow(keywords)
        all_jobs.extend(arb)

        prog.progress(25, text="Remotive (offres remote)...")
        rem = search_remotive(keywords)
        all_jobs.extend(rem)

        prog.progress(50, text="Adzuna (Indeed, Monster, Reed...)...")
        adz = search_adzuna(keywords, location, country, adzuna_id, adzuna_key)
        all_jobs.extend(adz)

        prog.progress(75, text="JSearch (LinkedIn, Glassdoor, ZipRecruiter...)...")
        jsr = search_jsearch(keywords, location, rapidapi_key)
        all_jobs.extend(jsr)

        prog.progress(100, text=f"✅ {len(all_jobs)} offres trouvées !")
        time.sleep(0.5)
        prog.empty()

    # Dédoublonnage approximatif par titre + entreprise
    seen = set()
    unique = []
    for j in all_jobs:
...
with st.expander("⚙️ Filtres avancés"):
    ...
    with fc2:
        filter_source = st.multiselect("Sources", ["Arbeitnow", "Remotive (Remote)", "Adzuna", "JSearch"])
    with fc3:
...
if search_btn and keywords:
    jobs = aggregate_jobs(
        keywords=keywords,
        location=location,
        country=country_codes.get(country, "fr"),
        adzuna_id=adzuna_id,
...
# ── Footer ─────────────────────────────────────────────────────────────────────
st.divider()
st.caption("AutoCandidature IA — Sources : Arbeitnow & Remotive (sans clé) · Adzuna · JSearch/RapidAPI (LinkedIn, Indeed, Glassdoor, ZipRecruiter) · Propulsé par Claude (Anthropic)")
